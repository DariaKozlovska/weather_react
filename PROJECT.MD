...
Aby uzyskać ocenę 3.0, nie trzeba korzystać z prawdziwych danych pogodowych, ale można jest _na sztywno_ wpisać do aplikacji. Ważne jednak, aby każda z miejscowości prezentowała **inne warunki pogodowe**.
...
-   dodanie możliwości wyboru (np. przy pomocy ikony _gwiazdki_ wyświetlanej przy każdej z miejscowości) i zapisywanie tych informacji w globalnym stanie aplikacji (Redux),
...
W projekcie można wykorzystać różne biblioteki dedykowane dla ekosystemu React'a. Poniżej znajduje się kilka z nich (_dla chętnych_):
